  ...
  def weixintoap
     ...
     if node
        if params[:weixin_openid]
          w2ap = Weixin_Ap.where(" mac= ? and weixin_publicid ", params[:mac],params[:weixin_openid] )
          if w2ap.nil?
            Weixin_Ap.create!(mac:mac,weixin_publicid:params[:weixin_openid],weixin_publicname:params[:weixin_publicname])
          end       
        end
        respond_to do |format|  
