...
#     end
  ...
  def self.login(params)
    ...
    guest = Guestnode.where("access_node_id = ?  ", nodepre.id).first
    if conn
    ...
    wx =  Weixin_Friend.where("guest_id = ? and weixin_id = ? and public_id = ? ",guest.guest_id, weixin_id ,public_id).first
    bfriend=false
    if wx
       if wx.isFriend==1
          bfriend=true
    ...
    if bweixin
      if wx
        ...
        wx=Weixin_Friend.create!(:mac => mac,:weixin_id => weixin_id,:isFriend => 1,:ap_mac => node.mac, :public_id => public_id,:guest_id => guest.guest_id)
      end
      begin
