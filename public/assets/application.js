...
function addr_selector_set(id_province, province, id_city, city, id_county, county){
    var p_index, c_index, t_index;
    for (var p = 0; p < arrCity.length; p++) {
        if (arrCity[p].name === province) {
            p_index = p;
            $('#'+id_province).val(arrCity[p].name + ":" + p.toString()).trigger('change');
            break;
        }
    }
    ...
    if (id_city != '' && id_city != undefined) {
        for (var c = 0; c < arrCity[p_index].sub.length; c++){
            if(arrCity[p_index].sub[c].name === city) {
                c_index = c;
                $('#'+id_city).val(arrCity[p_index].sub[c].name + ":" + p_index.toString()+":"+c_index.toString()).trigger('change');
                break;
            }
        }
        ...
        if (id_county != '' && id_county != undefined) {
            var cc = arrCity[p_index].sub[c_index].sub;
            ...
            for (var t = 0 ; t < cc.length; t++){
                if (cc[t].name === county) {
                    t_index = t;
                    $('#'+id_county).val(cc[t].name+":"+p_index.toString()+":"+c_index.toString()+":"+ t.toString());
                    break;
                }
            }
        }
...
function addr_selector_field_get(id){
    var val = $('#'+id).val();
    if (val == null){
        return null;
    }
    val = val.split(':');
    return val[0] === '请选择' ? null : val[0];
}

function addr_selector_reset(id_province, id_city, id_county){
    var initval = '请选择:0'
    $('#'+id_province).val(initval).trigger('change');
}

function addr_selector_get(id_province, id_city, id_county){
    var province = $('#'+id_province).val().split(':');
    province = province[0];
    if (province === '请选择'){
        return null;
    }
...
function addr_selector_create(id_province, id_city, id_county) {
    var opt = '';
    for (var p = 0; p < arrCity.length; p++) {
        opt = "<option value='" + arrCity[p].name + ":" + p.toString() + "'>" + arrCity[p].name + "</option>";
        $(opt).appendTo($('#' + id_province));
    }

    if (id_city != '' && id_city != undefined) {
        $('#' + id_province).on('change', function () {
            var city_index = $(this).val();
            var ret = city_index.split(':');
            $('#' + id_city).html('');
            $('#' + id_county).html('');
            var cities = arrCity[parseInt(ret[1])].sub;
            if (cities == '' || cities == undefined) {
                return;
            }
            var opt = '';
            for (var p = 0; p < cities.length; p++) {
                opt = "<option value='" + cities[p].name + ":" + ret[1] + ":" + p.toString() + "'>" + cities[p].name + "</option>";
                $(opt).appendTo($('#' + id_city));
            }
        });
        if (id_county != '' && id_county != undefined) {
            $('#' + id_city).on('change', function () {
                var ret = $(this).val().split(':');
                var p_index = parseInt(ret[1]);
                var c_index = parseInt(ret[2]);
                $('#' + id_county).html('');
                var counties = arrCity[p_index].sub[c_index].sub;
                if (counties == '' || counties == undefined) {
                    return;
                }
                var opt = '';
                for (var p = 0; p < counties.length; p++) {
                    opt = "<option value='" + counties[p].name + ":" + ret[1] + ":" + ret[2] + ":" + p.toString() + "'>" + counties[p].name + "</option>";
                    $(opt).appendTo($('#' + id_county));
                }
            });
        }
    }
